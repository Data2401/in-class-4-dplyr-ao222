...
starwars %>% na.omit(height) %>% group_by(eye_color) %>% summarize(mean(height))
```
...
```{r}
# talles male
starwars %>% na.omit(height) %>% summarize(max(height))
# how many blue eyed women
starwars %>% filter(eye_color == "blue", gender == "feminine") %>% count()
unique(starwars$gender)
# number of genders
unique(starwars$gender)
#at least two, possibly more because of NA values
```
